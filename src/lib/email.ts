...
type EmailDetails = { bookingBlock:string; arrivalTime:string; eventName:string; eventAge:string; secondName:string; secondAge:string; children:number; adults:number; quoteTotal:number };
const notice = [
...
function quoteNumber(b:Booking){return b.quoteNumber??"—";}
function summaryRows(b:Booking,d:EmailDetails):(string|number)[][]{return [["Numero preventivo",quoteNumber(b)],["Data",b.date],["Blocco",blockLabel(d.bookingBlock)],["Orario di arrivo",d.arrivalTime||"—"],["Nome festeggiato",d.eventName||"—"],["Età",d.eventAge?`${d.eventAge} anni`:"—"],...(d.secondName?[["Altro festeggiato",d.secondName],["Età altro festeggiato",d.secondAge?`${d.secondAge} anni`:"—"]]:[]),["Partecipanti",`${b.participants} totali (${d.children} bambini, ${d.adults} adulti)`],["Richiedente",b.name],["Telefono",b.phone],["E-mail",b.email],["Totale indicativo",`CHF ${d.quoteTotal}`]];}
function subject(b:Booking,d:EmailDetails){return `${b.date} · ${d.eventName||b.partyType} · ${b.participants} partecipanti · Prev. ${quoteNumber(b)} · CHF ${d.quoteTotal}`;}
function html(b:Booking,d:EmailDetails){const rows=summaryRows(b,d);return `<!doctype html><html><body style="font-family:Arial,sans-serif;background:#f6f7f2;padding:24px;color:#0c0d0b"><div style="max-width:720px;margin:auto;background:white;border-radius:24px;overflow:hidden;border:1px solid #ddd"><div style="padding:24px;background:linear-gradient(90deg,#ff6b21,#a6ff00)"><h1 style="margin:0;font-size:24px">Momòpolis – Riepilogo richiesta</h1></div><div style="padding:24px"><p>È stata completata una nuova richiesta dal preventivatore. Di seguito trovi il riepilogo completo delle informazioni selezionate.</p><table style="width:100%;border-collapse:collapse">${rows.map(([k,v])=>`<tr><th style="text-align:left;padding:10px;border-bottom:1px solid #eee;width:38%">${esc(k)}</th><td style="padding:10px;border-bottom:1px solid #eee">${esc(v)}</td></tr>`).join("")}</table><h2 style="margin-top:28px">Dettagli compilati</h2><pre style="white-space:pre-wrap;background:#f6f7f2;padding:16px;border-radius:14px;font-family:Arial,sans-serif">${esc(b.message||"")}</pre><div style="margin-top:24px;padding:18px;border:2px solid #ff6b21;border-radius:14px;font-weight:bold">${notice.map(x=>`<p style="margin:8px 0">• ${esc(x)}</p>`).join("")}</div><p style="margin-top:24px"><b>Contattare il cliente entro 24 ore</b> per confermare la festa e discutere i dettagli.</p><p><b>Momòpolis – Family Bar & Park</b></p></div></div></body></html>`;}
function text(b:Booking,d:EmailDetails){return [`RIEPILOGO RICHIESTA MOMÒPOLIS`,...summaryRows(b,d).map(([k,v])=>`${k}: ${v}`),"",b.message||"","",...notice.map(x=>`• ${x}`)].join("\n");}
