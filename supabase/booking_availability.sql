...
begin
  ...
  if not v_slot.is_available
    or v_slot.date < current_date + coalesce((v_config->>'minimumAdvanceDays')::integer, 5)
    or v_slot.date < v_start_date
    or extract(dow from v_slot.date) = 1
    or (v_end_date is not null and v_slot.date > v_end_date) then
    raise exception 'AVAILABILITY_CLOSED';
  end if;

  v_daily_limit := case
    when extract(dow from v_slot.date) in (0, 6)
      or coalesce(v_config->'holidayDates', '[]'::jsonb) ? v_slot.date::text
    then coalesce((v_config->>'weekendMaxBookings')::integer, 4)
    else coalesce((v_config->>'weekdayMaxBookings')::integer, 3)
  end;

  select count(*) into v_daily_bookings
  from bookings
  where date = v_slot.date and status <> 'cancelled';

  if v_daily_bookings >= v_daily_limit then
  ...
  insert into bookings (
    ...
    p_name, p_email, p_phone, p_party_type, p_participants, p_message, p_locale, 'pending'
  )
